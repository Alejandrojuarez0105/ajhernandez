  ...
  return (
    <section className="relative min-h-screen flex flex-col items-center justify-center bg-[#020d18] px-6 py-24 font-mono">
      ...
      <nav className="absolute top-0 left-0 right-0 flex items-center justify-between px-8 py-5 border-b border-[#1e3a5f]">
        <span className="text-[#ff8800] text-sm tracking-widest">aehernandez.dev</span>
        <div className="hidden md:flex gap-6">
          {['inicio','sobre mí','stack','proyectos','contacto'].map(s => (
            <a key={s} href={`#${s.replace(' ','-')}`}
      ...
      <div className={`w-full max-w-2xl flex flex-col items-center gap-7 transition-opacity duration-700 ${visible ? 'opacity-100' : 'opacity-0'}`}>
        ...
        <div className="w-full bg-[#060d18] border border-[#1e3a5f] rounded-xl overflow-hidden">
          <div className="flex items-center gap-2 bg-[#0d1525] border-b border-[#1e3a5f] px-4 py-2.5">
            ...
            <span className="text-[#475569] text-xs ml-2 tracking-widest">terminal — aehernandez.dev</span>
          </div>
          <div className="p-5 text-sm leading-loose">
            <p><span className="text-[#ff8800]">const</span> <span className="text-[#e2f0ff]"> alejandro</span><span className="text-[#ff8800]"> = {'{'}</span></p>
            <p>&nbsp;&nbsp;<span className="text-[#ff8800]">role</span><span className="text-[#ff8800]">:</span><span className="text-[#e2f0ff]"> "Full Stack Developer & DB Engineer"</span><span className="text-[#ff8800]">,</span></p>
            <p>&nbsp;&nbsp;<span className="text-[#ff8800]">frontend</span><span className="text-[#ff8800]">:</span><span className="text-[#e2f0ff]"> ["React", "TypeScript", "Vite", "HTML", "CSS"]</span><span className="text-[#ff8800]">,</span></p>
            <p>&nbsp;&nbsp;<span className="text-[#ff8800]">backend</span><span className="text-[#ff8800]">:</span><span className="text-[#e2f0ff]">  ["Node.js", "Python", "Java", "C#"]</span><span className="text-[#ff8800]">,</span></p>
            <p>&nbsp;&nbsp;<span className="text-[#ff8800]">databases</span><span className="text-[#ff8800]">:</span><span className="text-[#e2f0ff]"> ["MySQL", "PostgreSQL", "MongoDB", "Neo4j", "Supabase"]</span><span className="text-[#ff8800]">,</span></p>
            <p>&nbsp;&nbsp;<span className="text-[#ff8800]">available</span><span className="text-[#ff8800]">:</span><span className="text-[#ff8800]"> true</span></p>
            <p><span className="text-[#ff8800]">{'}'}</span></p>
            <p className="mt-2"><span className="text-[#334155]">$ </span><span className="text-[#ff8800]">_</span><span className="inline-block w-2 h-3.5 bg-[#ff8800] ml-0.5 animate-pulse align-middle" /></p>
          </div>
        </div>

        {/* Tagline */}
        <p className="text-[#475569] text-sm text-center leading-relaxed max-w-md">
          Construyo soluciones completas: desde la base de datos hasta la interfaz.<br />
        ...
        <div className="flex gap-8 flex-wrap justify-center pt-6 border-t border-[#1e3a5f] w-full">
          {[
            ...
            <div key={lbl} className="text-center">
              ...
              <p className="text-[#475569] text-xs mt-1 tracking-widest">{lbl}</p>
            </div>
          ))}
        </div>
